...
tokenising meta-events in Sect.~\ref{sec:token-comp-token-graphs}.

Additionally, we incorporate off-chain data from
CoinGecko~\cite{gecko-labs-xx} and DEX
...
Additionally, the first special case highlighted in
Sect.~\ref{sec:token-comp-evm-logs} is stipulated by
...
However, EVM logs for the Ethereum blockchain are generally accurate
and malicious contracts can be easily excluded.  For an aggregated
analysis, such as ours, the impact should be minimal.
...
being tracked.  It is also possible that CoinGecko's and DEX
Screener's coverage is incomplete or inaccurate.  However, as a
